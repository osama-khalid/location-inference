...
class training(object):  
    ...
    def timeGenerator(self,time,window):
        ...
        '''
        t=[time]
        time=datetime.strptime(time+":00:00", '%Y-%m-%d %H:%M:%S')
        window=int(window)
        for w in range(1,window+1):
            t.append((time-timedelta(hours=-w)).strftime("%Y-%m-%d %H:%M:%S").split(":")[0])
...
if __name__=="__main__":    
    ...
    j=maxWord
    i=window
    print("\nStarted for "+str(window)+" Hour window and maxwords = "+str(j))
