...
class training(object):  
    ...
    def wordSet(self,users,data,t,window):          #Generates the Set of Words time->city->word->count
        ...
        for row in data:
            if row[2] in users:
                ...
                for x in Time:
                    ...
                    if x in t:
                        if x not in WordPrior:      #Dat's Max Word
                            WordPrior[x]={}         #Dat's Max Word
                        ...
                        if city not in t[x]:
                            t[x][city]={}
                        for w in wordlist:
                            if w not in WordPrior[x]:           #Dat's Max Word
                                WordPrior[x][w]={}              #Dat's Max Word
                            if city not in WordPrior[x][w]:        #Dat's Max Word
                                WordPrior[x][w][city]=0            #Dat's Max Word
                            WordPrior[x][w][city]=WordPrior[x][w][city]+1     #Dat's Max Word
        ...
        ProbWord={}
        for x in WordPrior:
            if x not in ProbWord:
                ProbWord[x]={}
            for w in WordPrior[x]:
                if w not in ProbWord[x]:
                    ProbWord[x][w]=0
                    
                for c in WordPrior[x][w]:
                    ProbWord[x][w]=ProbWord[x][w]+(float(priors[x][c])/float(sum(priors[x].values())))*(float(WordPrior[x][w][c])/float(sum(WordPrior[x][w].values())))
        
        
        return(t,priors,ProbWord)
                ...
                if time in trained:
                    ...
                    for c in trained[time]:
                        ...
                        for w in wordlist:
                            ...
                            if mxW == 0:
                                postPred[c]=postPred[c]*featLike[w]
                            else:
                                if w in priorWord[time]:
                                    featLike[w]=featLike[w]*float(prior[time][c])/float(sum(prior[time].values()))/priorWord[time][w]
                                else:
                                    featLike[w]=featLike[w]*float(prior[time][c])/float(sum(prior[time].values()))
    ...
    for a in i:
        for b in j:
            ...
            for c in k:
                ...
                #shutil.rmtree(FILE+"_IA_W_"+str(a)+'_MX_'+str(b)+'.csv'+str(c))  
                for row in d:
                    if len(row)>0:
                        A.append(row)
