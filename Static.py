...
class training(object):  
    ...
    def dataStructure(self,userList,data):
        ...
        '''
           #t = dictionary of city->word->count
        trainedModel=self.wordSet(userList,data)        
 
        return(trainedModel)
        
        
    def wordSet(self,users,data):          #dictionary t,city->word->count
        '''
        ...
        for row in data:
            if row[2] in users:
                ...
                for w in wordlist:
                    ...
                    if w not in model[city]:
                        model[city][w]=0
                    model[city][w]=model[city][w]+1
        ...
        return(model,cityPriors,wordProbs)


class testing(object):
    def __init__(self,type,state,i,maxword,w=-1):
        part=i
...
if __name__=="__main__":    
    ...
    print("\nStarted for Static Day window and maxwords = "+str(j))
    ...
    combine("Words",type,state,-1,maxWord)       
    combine("Predicted",type,state,-1,maxWord)
